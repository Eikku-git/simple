...
#define Cast(x) static_cast<T>(x)
	...
	struct Mat3_T {
		...
		static constexpr inline Mat3_T& Transpose(Mat3_T& a) noexcept {
			simple::Swap(a[0][2], a[2][0]);
			simple::Swap(a[0][1], a[1][0]);
			simple::Swap(a[1][2], a[2][1]);
			return a;
		}
	...
	struct Mat4_T {
		...
		constexpr inline Mat4_T(T n0, T n1, T n2, T n3, T n4, T n5, T n6, T n7, T n8, T n9, T n10, T n11, T n12, T n13, T n14, T n15) noexcept
			: columns{ { n0, n1, n2, n3 }, { n4, n5, n6, n7 }, { n8, n9, n10, n12 }, { n13, n14, n15 } } {}
		...
		static constexpr inline Mat4_T LookAt(const Vec3_T<T>& eyePosition, const Vec3_T<T>& upDirection, const Vec3_T<T>& lookAtPosition) noexcept {
			...
			Vec3_T<T> right = Vec3_T::Cross(upDirection.Normalized(), front).Normalized();
			Vec3_T<T> up = Vec3_T::Cross(front, right).Normalized();
			Mat4_T result{};
			result[0].x = right.x;
			result[1].x = right.y;
			result[2].x = right.z;
		...
		constexpr inline Vec4_T<T>& operator[](size_t index) {
			assert(index < size); return columns[index];
		}
